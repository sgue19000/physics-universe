...
export function Lab({ concept }: { concept: Concept }) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (s.playing) step(concept.slug, s, dt);
    ...
    <div className="grid gap-4 lg:grid-cols-[220px_minmax(0,1fr)_300px]">
      ...
      <section>
        <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
          <div>
            <p className="text-xs uppercase tracking-widest text-zinc-500">{concept.category} · {level}</p>
        ...
        <div className="overflow-hidden rounded-lg border border-line bg-ink">
          <canvas ref={canvasRef} className="h-[320px] w-full touch-none md:h-[420px]" role="img" aria-label={`${concept.title} simulation canvas`} />
        </div>
        <div className="mt-3 flex flex-wrap gap-2 text-sm">
          <button className="rounded border border-line px-3 py-1" onClick={() => { s.playing = !s.playing; setTick((n) => n + 1); }}>{s.playing ? "Pause" : "Play"}</button>
          ...
          <label className="flex items-center gap-2 text-zinc-400">speed <input type="range" min={0.25} max={4} step={0.25} value={s.speed} onChange={(e) => { s.speed = Number(e.target.value); setTick((n) => n + 1); }} /> {s.speed}×</label>
        ...
        {mode === "challenge" && (
          <div className="mt-4 rounded border border-line p-3 text-sm">
            ...
            {revealed && <p className="mt-2 text-zinc-300">Change a slider and compare with the equations. The lab shows the physics; it does not grade you.</p>}
          </div>
        )}
        {mode === "experiment" && (
          <div className="mt-4 rounded border border-line p-3 text-sm">
